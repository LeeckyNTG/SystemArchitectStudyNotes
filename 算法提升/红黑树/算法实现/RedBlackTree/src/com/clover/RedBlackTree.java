...
public class RedBlackTree<T extends Comparable<T>, D> {
     ...
     * @return
     */
    public RBNode<T, D> getParent(RBNode<T, D> node) {
        if (node != null) {
            return node.parent;
        }
        return null;
     ...
     * @return
     */
    public Boolean getColor(RBNode<T, D> node) {
        if (node != null) {
            return node.color;
        }
        return BLACK;
     ...
     * @param x
     */
    public void RotateLeft(RBNode<T, D> x) {

        //右孩子
        RBNode<T, D> y = x.right;
    ...
    /**
     * 右旋
     * （当前节点就是父亲节点，整体过程就是 父亲下沉，左孩子上升，然后左孩子的左节点变成了原父亲的左节点）
     *
     * @param x
     */
    public void RotateRight(RBNode<T, D> x) {
        ...
        if (node.left != null && node.right != null) {
            ...
            if (node == getParent(replace)) {
                ...
                if (child != null) {
                    ...
                    //所以要为替换节点的孩子节点找到新父亲
                    setParent(child, getParent(replace));
                }
            ...
            for (int j = 0; j < map.get(i).size(); j++) {
                ...
                if (map.get(i).get(j).key != null) {
                    ...
                    if (map.get(i).get(j).right != null) {
                        System.out.println(map.get(i).get(j).key + "->" + map.get(i).get(j).right.key + "[label=right]");
                    }
                }
            }
        }
